#!/usr/bin/env python3
"""
Script to interact with Claude using PTY and automatically respond to permission prompts.
...
class ClaudePTYAutoResponder:
    """Handles automated responses to Claude's permission prompts using PTY."""
    ...
    def detect_permission_prompt(self, text):
        ...
        clean_text = self.remove_ansi_codes(text)
        return bool(self.permission_pattern.search(clean_text))
    ...
    def run_claude_interactive(self, path, initial_input, wait_time=5, timeout=300):
        """
        Run Claude interactively with PTY and respond to permission prompts.
        ...
        self.log("Starting Claude subprocess with PTY", feature="startup", module="main", path=str(path), input=initial_input)
        ...
        self.log("Creating subprocess with PTY", feature="startup", module="main", command=claude_path)
        ...
        try:
            ...
            self.log("Subprocess created", feature="startup", module="main", pid=process.pid)
        ...
        try:
            while True:
                ...
                if not initial_sent:
                    ...
                    if should_send:
                        ...
                        if elapsed - last_wait_message >= 1:
                ...
                try:
                    ...
                    if data:
                        ...
                        # Extract clean text (remove ANSI codes) for display
                        clean_data = self.remove_ansi_codes(data)
                        if clean_data.strip():
                            clean_output.append(clean_data)
                            # Print clean output in real-time
                            print(clean_data, end='', flush=True)
                        
                        # Check for permission prompt in buffer
                        if self.detect_permission_prompt(buffer):
                            self.log("Permission prompt detected", feature="permission", module="detector")
                            time.sleep(0.5)  # Wait for full prompt
                    ...
                    self.log("Error reading output", feature="error", module="reader", error=str(e))
            ...
            self.log("Cleanup complete", feature="lifecycle", module="main", return_code=return_code)
        
        # Return full output (with ANSI codes) and clean output
...
def main(path, input_string, wait_time, timeout, verbose, save_to, save_logs):
    """
    Run Claude with PTY and automatic response to permission prompts.
    
    When Claude asks "Do you want to...", this script automatically responds with "1".
    
    Examples:
        python claude_auto_responder_pty.py /home/user/project "Implement feature X"
        python claude_auto_responder_pty.py . "Fix the bug" --verbose
    """
    try:
        # Create auto-responder instance
        responder = ClaudePTYAutoResponder(verbose=verbose)
